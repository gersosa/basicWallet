...
class OperationSerializer(serializers.ModelSerializer):
    ...
    to_wallet = WalletSerializer(required=False)
    mount = WalletSerializer(required=False)

    def create(self, validated_data):
        data = self.context['request'].POST
        try:
            from_wallet = Wallet.objects.get(pk=data['wallet_from'])
            to_wallet = Wallet.objects.get(pk=data['wallet_to'])
